...
# directory to Plots
setwd("~/Documents/GitHub/NHANES_T2D/Plots")
...
# We 'convert' the list into a big dataframe, where each individuals is present 15-times
# This happens because we have 15 imputations of our dataset

result <- dplyr::bind_rows(imps_data)  

# Check the dimension of the result dataframe

dim(result) # 233670 rows and 14 variables, 233670 entries / 15imputations = 15578 entries per imp.

# Check the class of result (it's a dataframe, automatically converted by dplyr)

class(result)


# Let's now calculate the mean predicted risk per individual
# Each i has 15 different predictions, so: Sum(Predicted) / 15 gives us the mean

k <- result |> 
  mutate(id = rep(1:15578,15)) |> # That's a pseudo-id for each person, so we can group.
  group_by(id) |> 
  summarize(Risk_Framingham = mean(Risk_Framingham), Risk_ARIC = mean(Risk_ARIC)) |> 
  ungroup()

# Bind the columns of ethnicity and national screening
k <- bind_cols(k,imps_data[[1]]$ethnicity, imps_data[[1]]$National_Screening)

colnames(k)[4:5] <- c('Ethnicity','National_Screening')

cor(k$Risk_Framingham, k$National_Screening) #0.39
cor(k$Risk_ARIC, k$National_Screening) # 0.56

# Factor process for variables
k$National_Screening <- as.factor(k$National_Screening)
k$Ethnicity <- as.factor(k$Ethnicity)
k$Ethnicity <- droplevels(k$Ethnicity)
levels(k$Ethnicity)[1] <- 'Non-Hispanic Whites'
levels(k$Ethnicity)[2] <- 'Non-Hispanic Blacks'


# Create the plots

p1 <- ggplot(data = k, aes(x=Risk_Framingham, y=National_Screening)) +
...
p2 <- ggplot(data = k, aes(x=Risk_ARIC, y=National_Screening)) +
  geom_boxplot(aes(col = Ethnicity), outlier.shape = NA, lwd=1.25) +
...
# Some regressions (Keep those above risk score of 5 in screening program)

k_beta <- k |> filter(National_Screening %in% c('6','7','8','9'))

levels(k_beta$National_Screening)


# Run a linear regression model (Regressing risk of aric on ethnicity for those above score of 5 in screening)

model <- lm(Risk_ARIC ~ Ethnicity, data = k_beta)
...
# Run a linear regression model (Regressing risk of framingham on ethnicity for those above score of 5 in screening)

model1 <- lm(Risk_Framingham ~ Ethnicity, data = k_beta)
...
# Regression for those 5 or below in screening

k_beta1 <- k |> filter(National_Screening %in% c("-1","0", "1", "2", "3", "4", "5"))


# Aric
model3 <- lm(Risk_ARIC ~ Ethnicity, data = k_beta1)

summary(model3)

# Framingham

model4 <- lm(Risk_Framingham ~ Ethnicity, data = k_beta1)

summary(model4)

# For those 5 or below, ethinicity differences are not seem to present according to the data
broom::tidy(model4,conf.int = T)
